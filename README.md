...
Dependency-free **bulk domain availability checking** over ~69 verified TLDs (`.com`, `.ai`, `.ca`, `.io`, `.dev`, `.uk`, `.de`, `.fr`, `.au`, `.shop`, …), using authoritative registry data (RDAP) rather than scraping registrar websites or trusting search results. Available as an **npm CLI/library** (Node 18+) and an identical **Python script** (3.8+).
...
`python test_tlds.py` validates any TLD list against control domains (a known-registered name — google.tld or the ICANN-mandated nic.tld — must report `registered`, gibberish must report `available`). All 69 currently pass:
...
**ccTLD:** `ai ca io co me sh so gg us uk de fr nl eu be at it pt se no dk fi pl cz ie au nz in br mx sg tv cc`
